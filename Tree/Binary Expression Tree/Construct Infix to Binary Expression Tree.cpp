...
int main(){
    ...
    cout << name << ": " << infix << endl;

    string result = postfix(infix);
    bool error = (result == "!") ? true : false;
    if(!error){
        Expression *express = new Expression;
        express = expression(result);
        error = (!express->valid) ? true : false;
        if(!error){
            NODE *binaryTree = binaryExp(result);
